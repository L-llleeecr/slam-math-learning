...
第三章将在此基础上加入局部欧氏性、Hausdorff性和第二可数性，定义拓扑流形。
...
这个结论是第三章中采用Hausdorff和第二可数条件的重要原因之一：它保证局部坐标、局部函数和局部几何对象可以通过良好控制的覆盖拼成全局对象。
...
进入第三章后，拓扑空间$M$还要满足局部欧氏性、Hausdorff性和第二可数性：
\[
    \forall p\in M,\quad
