...
\begin{definition}[切空间中的局部残差]
固定当前估计$g\in G$，定义
\[
    \widetilde r_g:U\subset\mathfrak g\to\R^m,
    \qquad
    \widetilde r_g(\delta\xi)=r(g\Exp(\delta\xi)),
\]
其中$U$是$0\in\mathfrak g$附近的邻域。
\end{definition}
...
\[
    \widetilde r_g(0)=r(g).
\]
由于$\widetilde r_g$的定义域是线性空间$\mathfrak g$，可以在$0$处进行普通的一阶线性化：
\[
    \widetilde r_g(\delta\xi)
    \simeq
    r(g)+D\widetilde r_g\big|_0[\delta\xi].
\]
...
现在改用$u$描述同一个变量$x$。设
\[
    \phi:V\subset\R^k\to U,
    \qquad
    x=\phi(u),
\]
并令$r:U\to\R^m$。真正关于$u$求导的函数是复合映射
\[
    V\xrightarrow{\ \phi\ }U\xrightarrow{\ r\ }\R^m.
\]
链式法则给出
\[
    \boxed{
    d(r\circ\phi)_u
    =dr_{\phi(u)}\circ d\phi_u.
    }
\]
在标准坐标下，这才写成常见的矩阵乘法
\[
...
所以
\[
    \frac{d(r\circ\phi)}{du}
    =\frac{dr}{dx}\frac{dx}{du}
    =2e^{2u}.
\]
同一个局部变化用$\delta x$或$\delta u$表示时满足
\[
    \delta x\simeq J_\phi(u)\delta u,
    \qquad
    \delta r\simeq J_r(x)J_\phi(u)\delta u.
\]
所以Jacobian始终相对于所选局部参数而言，这一问题并不是到了李群才出现。
\end{lieexample}

若$\phi$是局部微分同胚，$x=\phi(u)$，则还要区分正向与逆向换元：
\[
    d\phi_u:T_u\R^n\to T_x\R^n,
    \qquad
    d(\phi^{-1})_x:T_x\R^n\to T_u\R^n,
\]
且
\[
    J_{\phi^{-1}}(x)=J_\phi(u)^{-1}.
\]
...
\[
    r:\SE(3)\to\R^m.
\]
对$\delta\xi\in\se(3)=T_I\SE(3)$，不能写$T+\delta\xi$，而要选择从$0\in\se(3)$到$T$邻域的局部参数化。按扰动放置侧记为
\[
