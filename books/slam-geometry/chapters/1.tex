...
这些问题要等第三章和后续流形章节为$\mathcal X$赋予额外结构后再回答。
...
\]
第三章为商集和$S^1$分别赋予相应拓扑后，这个双射会成为同胚：
\[
...
本章只保留这个结构预告，不展开$\Phi$的矩阵公式。第五章会讨论四元数双覆盖， 第六章会直接使用$\SO(3)$和$\SE(3)$的李群结构。
...
这些例子只使用本章的集合和映射语言。连续性、开集、紧致性和局部坐标将在第三章和第四章中继续添加。
